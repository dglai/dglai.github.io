...
implementations of Graph Neural Networks. We decide to keep DGL
framework-agnostic to engage with users from different platforms (PyTorch,
MXNet...). As a result, in our earlier releases, we largely leverage the
...
we have been working on boosting its performance in a high gear. As per our
next major release (**v0.3**), we are pleased to offer much faster training
...
![](https://i.imgur.com/wTda40k.jpg){: .aligncenter}
...
`send_and_recv` (shown in following figure). Under the veil, the fused
...
* *How to fused the user-defined functions $\phi^e$ and $\Sigma$?* In DGL, we
...
[A Graph Attention Network (GAT)](https://docs.dgl.ai/tutorials/models/1_gnn/9_gat.html) can also be implemented using the built-in `src_mul_edge` message function and `sum` reduce function.
...
* Broadcasting is supported on the feature dimensions. For example, in
  multi-head attention model, the node feature is of shape `(NUM_HEADS,
...
We compare DGL with PyG (Pytorch Geometric v1.2.0). PyG implements basic
message passing by first `gather` node features as edge messages and then
`scatter` them for message aggregation, which generates explicit messages. We
also compare with our v0.2 release to see how much we have improved the system.
...
The up-coming release has huge performance boost especially on GAT model
(**19x**) faster due to the kernel fusion technique. Compared with PyG, for
small graphs (i.e, Cora, Citeseer and Pubmed), the computation and memory
consumption stay the same regardless of the graph sizes. In such regime, the
graph computation does not dominate the whole training and DGL has some slight,
constant overhead compared with PyG. However, on a much larger reddit graph,
PyG runs out of memory while DGL can fit the graph quite easily.
...
We have a bag of features we want to deliver on our agenda.  In fact, many of
them have been in our mind from the birth of DGL. For example, the built-in
functions have been in the project for a long time but only shine with the help
of kernel fusion. We want to stay more closely with the community so we want to
share some of the directions we are working on:
...
Lastly, try out this new feature and leave us your feedback! For now, you need
to clone DGL's repository, switch to `kernel` branch and build it from source.
Stay tuned for our next major release more updates on the DGL project.
